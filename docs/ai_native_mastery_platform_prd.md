...
The proposed product is a Google Cloud-supported, offline-first mastery and evidence engine designed to reduce that access gap. The Android learner app is the primary product and contains the local data, content, sandbox, mastery rules, and synchronization queue needed to complete the core Diagnose, Learn, Practice, Verify, and Revisit loop without an active connection. A separate web portal supports content authoring, evidence review, and pilot operations. Both clients use shared Firebase and Cloud Run services when connected; the mobile app synchronizes with those services rather than connecting to the web frontend. Google Cloud extends the experience but is not required to complete the downloaded core loop.
...
| Client architecture | Offline Android learner app and separate web operations portal on shared backend services | Keeps the learner experience independent of the web interface while allowing common content, review, synchronization, and cloud AI services.               |
...
## 8.8 Application component architecture

The product has two independent clients. The Android learner application is an offline-capable application, not a wrapper around the web portal. The web portal supports authors and reviewers. Each client communicates with the same authenticated cloud services according to its role, but neither client communicates through the other.

| **Component**         | **Primary responsibilities**                                                                                                                     | **Connected dependencies**                                                                                                                 | **Offline requirement**                                                                        |
|-----------------------|--------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------|
| Android learner app   | Learner interface, local profile, content packs, mastery loop, sandbox, evidence log, outbox, synchronization, and GenAI routing.                | Firebase for identity, content, event synchronization, and configuration; Cloud Run for protected operations; cloud Gemini when permitted. | Complete the downloaded core loop and preserve all work without a connection.                  |
| Web operations portal | Content and skill-graph authoring, rubric management, content-pack publishing, review queues, and pilot monitoring.                              | Firebase and Cloud Run shared with the learner app.                                                                                        | No offline requirement for the MVP; temporary outages must not affect active learner sessions. |
| Firebase services     | Authentication, App Check, synchronized records, file delivery, web hosting, feature configuration, and client access to eligible AI services.   | Google Cloud project services and application clients.                                                                                     | Not required during an active downloaded learning session.                                     |
| Cloud Run services    | Content-pack construction, secure assessment assignment, complex synchronization, AI orchestration, review workflows, and analytics aggregation. | Firebase data, approved content sources, and cloud Gemini.                                                                                 | Queue work until reconnection; never own the only copy of an unsynchronized learner event.     |
| Google AI services    | Cloud Gemini for richer tutoring and evidence review; Gemini Nano through supported Android on-device inference.                                 | Firebase AI Logic or protected server orchestration.                                                                                       | Optional enhancement only; authored deterministic behavior remains universal.                  |

## 8.9 Local ownership and synchronization flow

The Android local database is authoritative for unsynchronized learner activity. Firebase offline caching may improve responsiveness but is not the sole persistence mechanism for the learning loop. Every activity, assistance event, artifact reference, stage decision, and mastery update is committed locally before any network request. An append-only outbox then synchronizes events idempotently when a connection becomes available.

1\. The learner downloads a signed, versioned content pack while connected or receives it through an approved transfer path.

2\. The Android app completes activities locally and records evidence, assistance, policy version, and inference provenance before showing completion.

3\. The synchronization manager uploads stable event identifiers and resumes safely after interruption without duplicating progress.

4\. Shared services acknowledge accepted events, route provisional evidence for cloud or human review, and return reviewed results.

5\. The device downloads content, rubric, policy, and model-configuration updates without replacing historical evidence or its original versions.

6\. Conflicts, rejected events, and pending reviews remain visible; synchronization failure never silently discards or rewrites learner work.

Connection boundary. The mobile app may call Firebase directly for approved client operations and may call Cloud Run for protected domain operations. The web portal uses the same services for its authorized workflows. Sensitive logic, secrets, secure assessment material, cross-user queries, content signing, and cloud AI orchestration must remain on trusted server infrastructure.
...
| Android application shell                    | Native or cross-platform Android client with local database, content-pack manager, mastery engine, sandbox, append-only outbox, synchronization manager, capability detection, and GenAI gateway.                                                                   |
| Web operations portal                        | A small connected portal for content and rubric management, content-pack publication, required evidence review, and pilot monitoring. It is not required for learner session execution.                                                                             |
| Shared backend services                      | Firebase provides client-facing identity, protected data and file access, hosting, configuration, and synchronization support. Cloud Run owns sensitive domain operations, content packaging, complex review, analytics, and cloud AI orchestration.                |
...
| FR26   | Maintain a local Android database as the authoritative source for unsynchronized learner events and evidence.                                                             | Must         |
| FR27   | Write learner events to an append-only outbox before network submission and synchronize them idempotently with stable identifiers.                                        | Must         |
| FR28   | Provide a separate web portal for content publication, rubric management, required evidence review, and pilot monitoring.                                                 | Must         |
| FR29   | Route approved client operations through Firebase and protected domain operations through authenticated Cloud Run services.                                               | Must         |
| FR30   | Preserve original content, rubric, policy, model, and inference-location versions across synchronization and later review.                                                | Must         |
...
| Client separation       | The Android app and web portal are independent clients of shared services; loss of the web portal must not interrupt learner sessions.                                                                               |
| Offline data durability | A completed local event must survive process death, device restart, repeated synchronization, and temporary server rejection without duplication or silent loss.                                                     |
| Backend trust boundary  | Secrets, content signing, secure assessment allocation, cross-user access, and consequential review actions execute only on trusted server infrastructure.                                                           |
...
| 15 Application architecture         | A. Mobile wrapper around web app; B. Cloud-dependent mobile client; C. Offline Android client plus separate web portal using shared Firebase and Cloud Run services | Recommended: C. It protects the offline access promise, separates learner and operational interfaces, and concentrates sensitive logic in trusted backend services.                                                                                                                                     |
